...
class NewsFilteredRetriveAPIView(generics.RetrieveAPIView):
    queryset = News.objects.all()
    serializer_class = NewsSerializer
    permission_classes = (permissions.AllowAny,)
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            result = News.objects.filter(created_at__range = (start_date_aware, end_date_aware))

            if not result.exists():
                return Response({"result": "No news found in the specified date range."})
            
            serialized_result = NewsSerializer(result, many=True).data
...
class NewsRetrieveDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    queryset = News.objects.all()
    serializer_class = NewsSerializer
    permission_classes = (permissions.AllowAny,)
...
class LatestNewsListAPIView(generics.ListAPIView):
    serializer_class = NewsSerializer
    permission_classes = (permissions.AllowAny,)
